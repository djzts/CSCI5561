...
def conv_backward(dl_dy, x, w_conv, b_conv, y):
    ...
    x_shape = x.shape
    w_conv_shape = w_conv.shape
    dl_dy = np.transpose(dl_dy,(2,0,1))

    dl_dw = np.zeros_like(w_conv)
    dl_db = np.zeros_like(b_conv)

    dl_dy = dl_dy.reshape(x_shape[-1],x_shape[0]*x_shape[1])

    temp = np.zeros(x_shape[0]*x_shape[1],w_conv_shape[0]*w_conv_shape[1])

    x_padding = np.pad(x[:,:,0],(1,))
        for i in range(x_shape[0]):
            for j in range(x_shape[1]);
                temp = x_padding[i:i+w_conv_shape[0],j:j+w_conv_shape[1]].reshape((1,w_conv_shape[0]*w_conv_shape[1]))
    for k in range(x_shape[-1]):
        dl_dw[:,:,0,i] = (dl_dy[i] @ temp).reshape(x_shape[0],x_shape[1])
        dl_db[0, k] = np.sum(dl_dy[k])
...
def train_cnn(mini_batch_x, mini_batch_y):
    ...
    w_conv = np.random.normal(0,0.27,(3,3,1,3))
    b_conv = np.random.normal(0, 0.1,(1,3))
    w_fc = np.random.normal(0, 0.1,(10,147))
    b_fc = np.random.normal(0, 0.1,(10,1))
